...
public class Login extends JFrame implements View{
	...
	private void addL2(){
		...
		l2.setBounds((WIDTH/2)-((size.width+tf2.getPreferredSize().width)/2), HEIGHT/2+(l1.getPreferredSize().height*2),
		size.width, size.height);
	}	
	private void addTf1(){
		add(tf1);
		Dimension size = tf1.getPreferredSize();
		tf1.setBounds((WIDTH/2)+l1.getPreferredSize().width-((size.width+l1.getPreferredSize().width)/2), HEIGHT/2,
		size.width, size.height);
	}	
	private void addTf2(){
		add(tf2);
		Dimension size = tf2.getPreferredSize();
		tf2.setBounds((WIDTH/2)+l2.getPreferredSize().width-((size.width+l2.getPreferredSize().width)/2), HEIGHT/2+(l1.getPreferredSize().height*2),
		size.width, size.height);
	}	
	private void addB1(){
		add(b1);
